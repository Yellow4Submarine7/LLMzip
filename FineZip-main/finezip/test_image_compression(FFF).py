import os
import traceback
from image_compression_FFF import create_image_zip_model
...
def test_image_compression(image_path: str):
    ...
    # 创建输出目录
    os.makedirs("test_output_FFF", exist_ok=True)  # 使用不同的输出目录
    
    # 设置输出路径
    zip_path = os.path.join("test_output_FFF", "compressed.gpz")
    output_path = os.path.join("test_output_FFF", "decompressed.png")
    
    try:
        # 创建模型实例
        print("正在创建模型实例(FFF版本)...")
        model = create_image_zip_model("meta-llama/Llama-3.1-8B")
        
        # 压缩图片
        print(f"\n开始压缩图片(使用十六进制格式): {image_path}")
        ...
        # 添加与原始版本的比较
        if os.path.exists("test_output/compressed.gpz"):
            original_compressed_size = os.path.getsize("test_output/compressed.gpz")
            print("\n与原始版本比较:")
            print(f"原始版本压缩大小: {original_compressed_size/1024:.2f} KB")
            print(f"FFF版本压缩大小: {compressed_size/1024:.2f} KB")
            print(f"大小减少: {(original_compressed_size - compressed_size)/1024:.2f} KB")
            print(f"压缩率提升: {(original_compressed_size - compressed_size)/original_compressed_size*100:.2f}%")
