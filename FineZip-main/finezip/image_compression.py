...
class ImageZipModel(ZipModel):
    ...
    # 定义常量
    BLOCK_SIZE = 8  # 8x8像素块
    PIXEL_SEPARATOR = ','  # RGB值之间的分隔符
    BLOCK_SEPARATOR = ' | '  # 块之间的分隔符
            ...
            for i in range(0, height, self.BLOCK_SIZE):
                for j in range(0, width, self.BLOCK_SIZE):
                    ...
                    for pixel in block.reshape(-1, 3):
                        r, g, b = pixel
                        block_values.append(f"{r}{self.PIXEL_SEPARATOR}{g}{self.PIXEL_SEPARATOR}{b}")
        ...
        try:
            ...
            for i in range(0, height, self.BLOCK_SIZE):
                for j in range(0, width, self.BLOCK_SIZE):
                    ...
                    # 解析当前块的像素值
                    try:
                        pixels = pixel_blocks[block_idx].strip().split()
                        pixel_values = []
                        for p in pixels:
                            if p:  # 确保不是空字符串
                                r, g, b = map(int, p.split(self.PIXEL_SEPARATOR))
                        ...
                        logging.error(f"Block content: {pixel_blocks[block_idx][:100]}...")
    ...
    def decode_image(self, metadata: Dict[str, Any]) -> Image.Image:
        ...
        try:
            # 检查是否是分块数据
            if 'encoded_chunks' in metadata:
                print(f"分块数量: {len(metadata['encoded_chunks'])}")
                total_length = sum(len(chunk) for chunk in metadata['encoded_chunks'])
                print(f"编码数据总长度: {total_length}")
                
                # 解码所有块
                all_decoded = []
                for i, chunk in enumerate(metadata['encoded_chunks']):
                    print(f"\n开始解码第 {i+1}/{len(metadata['encoded_chunks'])} 块...")
                    chunk_start_time = time.time()
                    
                    # 将chunk转换为tensor并移到正确的设备上
                    chunk_tensor = torch.tensor(chunk, device=self.device)
                    decoded_chunk = self.decode_tokens(chunk_tensor)
                    all_decoded.append(decoded_chunk)
                    
                    chunk_time = time.time() - chunk_start_time
                    print(f"第 {i+1} 块解码完成，用时: {chunk_time:.2f}秒")
                
                # 合并所有解码后的块
                print("\n合并所有解码块...")
                decoded = torch.cat(all_decoded)
                print(f"合并后的token序列长度: {len(decoded)}")
            
            else:
                # 单块处理
                print("单块数据处理...")
                encoded = torch.tensor(metadata['encoded'], device=self.device)
                decoded = self.decode_tokens(encoded)
                
            # 转换回图片
            print("\n开始转换为图片...")
            img = self.tokens_to_image(decoded, metadata)
            
            end_time = time.time()
            total_time = end_time - start_time
            logging.info(f"Image decoding completed in {total_time:.2f} seconds")
            ...
            if 'encoded' in locals():
                logging.error(f"encoded device: {encoded.device}")
            if 'decoded' in locals():
                logging.error(f"decoded shape: {decoded.shape}")
            raise
    ...
    def unzip_image(self, zip_path: str, output_path: str):
        ...
        try:
            ...
            if 'encoded_chunks' in metadata:
                print(f"分块数量: {len(metadata['encoded_chunks'])}")
                print(f"编码数据长度: {sum(len(chunk) for chunk in metadata['encoded_chunks'])}")
                # 使用第一个块进行解码
                metadata['encoded'] = metadata['encoded_chunks'][0]
            else:
    ...
    def pad(self, tokens, padding_val):
        ...
        try:
            ...
            print(f"tokens device: {tokens.device if isinstance(tokens, torch.Tensor) else 'not a tensor'}")
            raise
